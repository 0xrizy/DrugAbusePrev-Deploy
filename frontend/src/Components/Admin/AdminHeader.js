...
const AdminHeader = () => {
  ...
  const handleLogout = () => {
    // Remove isAdminLoggedIn from local storage on logout
      ...
      <div className="container mx-auto flex items-center justify-between px-4">
        ...
        {isOpen && (
          <div className="lg:hidden absolute top-16 right-4 bg-gray-700 p-4 rounded">
            <Link to={"/admindashboard"} className="block text-gray-300 py-2">
              Administration
            </Link>
            <button
              onClick={handleLogout}
              className="block text-gray-300 py-2 cursor-pointer"
            >
              Logout
            </button>
          </div>
